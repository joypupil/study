package com.joypupil.study.test.文件加解密.三重DES加密;
	...
	private void encrypt(File fileIn, String sKey) {
		try {
			if (sKey.length() == 48) {
				...
				System.out.println("加密用时：" + (start - end));
				String fileOut = fileIn.getPath() + "en";
				...
				System.out.println("存储文件用时：" + (end - start));
			} else
				;
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			if (sKey.length() == 48) {
				...
				byte[] bytOut = decryptByDES(decryptByDES(decryptByDES(bytIn,
						bytK3), bytK2), bytK1);
//				File fileOut = new File("f:\\test\\test.txt");
